...
4. **Probabilistic Matching**. Three different supervised model are used to predict further matches among those records for the whom the automatic matching did not find N matches. The models can be trained using the $\texttt{./script/01\_training.py}$ script, and saved into the $\texttt{./models/}$ folder.
